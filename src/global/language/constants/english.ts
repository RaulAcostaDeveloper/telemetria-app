import { LanguageInterface } from "./language.model";

export const ENGLISH: LanguageInterface = {
  localeLanguage: "en-US",
  languages: {
    english: "English",
    spanish: "Spanish",
    portugues: "Portuguese",
  },
  languageButton: {
    selectLanguage: "Select language",
  },
  sectionName: {
    home: "Home",
    management: "Resources",
    fuel: "Fuel",
    telemetryobd: "Telemetry",
    iam: "Access control",
  },
  notifications: {
    unexpectedError: "Unexpected error",
    genericServiceError: "Failed to fetch data, trying again...",
    nullValue: "No information for this time period",
    graphicError: "Unable to display the chart",
    lessThanOneDay:
      "Please select a date range of more than one day to visualize data",
    confirmation: "Confirmation",
    noChanges: "No changes detected",
    savedData: "The changes have been saved",
    noRegisters: "There are no records that match the search parameters",
    dateRangeSelected: "A time range has been selected for the queries",
    tooManyRequest: "The maximum number of requests has been reached.",
    pleaseReTry: "Please try again after a few seconds.",
  },
  header: {
    backButton: {
      hover: "Go back",
    },
    vehicleFilter: {
      actionFuelTitle: "Go to fuel report",
      actionManagementTitle: "Go to vehicle management",
      actionTelemetryTitle: "Go to telemetry report",
      inputPlaceholder: "Search vehicle...",
      inputNoMatch: "No matches found",
    },
    calendar: {
      buttonHover: "Open calendar options",
      datePicker: {
        buttons: {
          today: "Today",
        },
      },
      fixedDateFilterOptions: [
        "Last 7 days",
        "Last 15 days",
        "Last 30 days",
        "Last 90 days",
        "This month",
        "Last month",
      ],
      reportingPeriod: "Reporting period",
      fromLabel: "From:",
      toLabel: "To:",
      acceptButtonLabel: "Accept",
      cancelButtonLabel: "Cancel",
      errorMessage3: `A "From" or "To" date was not found.`,
      errorMessage1: `The "To" date cannot be earlier than the "From" date.`,
      errorMessage2: `The time selected in the "To" date cannot be later than the current time.`,
      errorMessage: "Please select a valid time.",
      daysOfWeek: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
      daysOfWeekTitle: "Select",
      monthNames: [
        "January",
        "February",
        "March",
        "April",
        "May",
        "June",
        "July",
        "August",
        "September",
        "October",
        "November",
        "December",
      ],
    },
  },
  menu: {
    titles: {
      close: "Close menu",
      logo: "Company logo",
      open: "Open menu",
    },
    buttons: {
      logOut: "Log out",
    },
    modal: {
      titleLogout: "Confirmation",
      messageLogout: "Do you want to log out?",
      buttonCancel: "Cancel",
      buttonAccept: "Accept",
    },
  },
  table: {
    buttons: {
      cancel: "Cancel",
      create: "Create",
      delete: "Delete item",
      downloadCSV: "Export",
      downloadFiltredCSV: "Download CSV (filtered)",
      filtered: "Filtered",
      saveEdit: "Save edit",
      saveNew: "Save item",
      filtersButton: "Filters",
    },
    actions: {
      addProfileToZone: "Edit zone's profile",
      close: "Close",
      agree: "Accept",
      copy: "Copy text",
      copyError: "Copy failed",
      deleteElement: "Delete item",
      editElement: "Edit item",
      filterBy: "Filter by column",
      any: "No filter",
      search: "Search",
      searchTitle: "Search by first column",
      sortItems: "Sort items by",
      noImei: "This vehicle has no device linked",
      viewDetail: "View details",
      goFuelReport: "Go to fuel report",
      goObdReport: "Go to OBD report",
      goZoneReport: "Go to zone",
      goGenericReport: "Go to report",
      cleanFilters: "Clear",
      goPage: "Go to page",
      noFilters: "No filters available for this table",
    },
    elements: {
      actions: "Actions",
      columns: "Columns",
      total: "Total",
      average: "Average",
      validDataLength: "Valid values: ",
    },
    formTitles: {
      filters: "Filters",
      createElement: "Create item",
      deleteElement: "Are you sure you want to delete this item?",
      deleteSubString: "This action is permanent and cannot be undone.",
    },
  },
  highCharts: {
    options: {
      rangeSelectorShowAll: "Show all",
    },
    axisTitles: {
      fuelVariation: "Fuel level (L)",
      fuelVariationCAN: "Fuel level (L) (CAN)",
      time: "Time",
      performance: "Performance (L)",
      distance: "Distance (Km)",
      timeTraveled: "Working hours (H)",
      speed: "Speed (Km/h)",
    },
    titles: {
      charges: "Refuels",
      disCharges: "Discharges",
      fuelVariation: "Fuel level ",
      fuelVariationCAN: "Fuel level",
      performancesBetweenCharges: "Performance between charges",
      dailyPerformance: "Daily performance",
      speed: "Speed",
      engineOff: "Engine off",
      engineOnIdle: "Engine parked",
      engineOnMoving: "Engine running (moving)",
    },
    tooltips: {
      date: "Event date",
      startDate: "Start event date",
      endDate: "End event date",
      lat: "Latitude",
      lon: "Longitude",
      fuel: {
        odometer: "Odometer",
        address: "Address",
        speed: "Speed",
        ignition: "Ignition",
        on: "On",
        off: "Off",
        deviceBattery: "Battery",
        mainPower: "Voltage",
        totalCharges: "Total refuels",
        initialFuel: "Initial fuel",
        finalFuel: "Final fuel",
        totalDischarges: "Total discharges",
        averagePerformance: "Average performance",
        tanks: "Tank level",
        tanksSum: "Global fuel",
        fuelConsumed: "Fuel consumed",
        initialLevel: "Initial level",
        finalLevel: "Final level",
        initialOdometer: "Initial odometer",
        finalOdometer: "Final odometer",
        distanceTravelled: "Distance traveled",
        titleCharges: "Charge",
        titleDischarges: "Discharge",
        titleFuelVariationCAN: "Fuel level",
        titleFuelVariation: "Fuel level",
        titlePerformanceBetween: "Performance Between Charges",
        titlePerformanceDaily: "Daily Performance",
        titleSpeed: "Speed",
        subtitleCharges: "Details",
        subtitleDischarges: "Details",
        subtitleFuelVariationCAN: "Details",
        subtitlePerformanceBetween: "Details",
      },
      rpm: {
        rpm: "Revolutions per minute",
        titleTelemetryRPM: "Revolutions per Minute",
        subtitleTelemetryRPM: "Telemetry",
      },
      distance: {
        distance: "Distance",
        distanceTraveled: "Distance traveled",
        titleDistance: "Distance Traveled",
        subtitleDistance: "Telemetry",
      },
      timeTraveled: {
        timeTraveled: "Worked hours",
        titleTimeTraveled: "Worked Hours",
        subtitleTimeTraveled: "Telemetry",
      },
      telemetryOBD: {
        xAxisDistance: "Distance traveled (km)",
        xAxisDriven: "Time (h)",
        yAxis: "Vehicles",
      },
    },
  },
  geoModalTitles: {
    timeTraveledTitle: "Working hours report map",
    rpmTitle: "RPM report map",
    totalDistanceTitle: "Distance report map",
    fuelChargeTitle: "Refuel map",
    fuelDischargeTitle: "Discharge map",
    levelMessageTitle: "Level change map",
    fuelNowTitle: "Fuel report location",
    roadmap: "Standard map",
    satellite: "Satellite",
  },
  auth: {
    principalTitle: "Authentication page",
    authForm: {
      name: "Username",
      password: "Password",
      title: "Authentication",
      loginButton: "Log in",
      mayusActivated: "Caps lock is on",
    },
    authFormNote: {
      nameOrPasswordError: "Invalid username or password",
      networkError: "Network error",
      unexpectedError: "Unexpected internal error",
    },
    linkToHome: "Go to home",
    linkToRegister: "Go to register",
    checklogin: {
      maintitle: "Verifying session",
    },
  },
  fuel: {
    filter: {
      header: "Filters",
      showFilters: "Show filters",
      hideFilters: "Hide filters",
      account: "Account",
      group: "Group",
      brand: "Brand",
      model: "Model",
      selectAnOption: "Select an option",
      search: "Search",
    },
    tabs: {
      charges: "Refuels",
      unitys: "Units",
      discharges: "Discharges",
      zones: "Zones",
    },
    vehiclesTableColumns: {
      imei: "IMEI",
      lastFuelLevel: "Last fuel level",
      lastReportDate: "Last report date",
      name: "Name",
      plate: "Plate",
      fuelLoadCount: "Refuels",
      fuelUnloadCount: "Discharges",
      performanceOdometer: "Performance (Odo Km/L)",
      performanceHorometer: "Performance (Horo)",
      fuelLoaded: "Fuel filled",
      fuelUnloaded: "Fuel drained",
    },
    summaryReports: {
      labels: {
        inventory: "Total inventory",
        performance: "Average fleet performance",
        fuelCharged: "Fuel filled",
        fuelDischarged: "Fuel drained",
        unitsAnalyzed: "Units analyzed",
        totalDistanceTraveled: "Total distance traveled",
      },
    },
    donutGrpahic: {
      title: "Vehicle distribution",
      waitingMessage: "Loading 3D chart...",
      noDataMessage: "No data available",
      metric: "Metric",
      segments: "Segments",
      inventory: "Fuel",
      charged: "Refueled",
      discharged: "Discharged",
    },
  },
  fuelVehicle: {
    tabs: {
      behavior: "Behavior",
      reports: "Performance",
      fuelNow: "Current fuel",
      charges: "Fills",
      discharges: "Drains",
    },
    vehicleReports: {
      distanceEfficiency: "Efficiency",
      distanceTravelled: "Distance traveled",
      fuelConsumed: "Fuel consumed",
      fuelEnd: "Final fuel",
      fuelLoaded: "Fuel filled",
      fuelUnloaded: "Fuel unloaded",
      fuelStart: "Initial fuel",
      operatingHours: "Operating hours",
      maxSpeed: "Maximum Speed",
      timeEfficiency: "Time efficiency",
    },
    fuelNow: {
      lastDateReport: "Last report date",
      fuelNow: "Current fuel",
      capacity: "Capacity",
      velocity: "Speed",
      odometer: "Odometer",
      ignition: "Ignition",
      on: "On",
      off: "Off",
      tank: "Tank",
      showLocation: "Show map",
      hideLocation: "Hide map",
      stopSynchronization: "Syncing",
      startSynchronization: "Not syncing",
      startedSynch: "Synchronization has started",
      stoppedSynch: "Synchronization has stopped",
    },
    fuelChargesLabels: {
      eventId: "Event ID",
      startDate: "Start",
      endDate: "End",
      origin: "Origin",
    },
  },
  onBoardDiagnosticsVehicle: {
    tabs: {
      averageRpm: "RPM",
      analysis: "Analysis",
      totalDistance: "Distance traveled",
      totalTimeWorked: "Time worked",
    },
    analysisTab: {
      totalIdleTime: "Total engine parked time",
      allowedMaxSpeed: "Allowed max speed",
      maxSpeed: "Max speed",
      averageRpm: "Average RPM",
      plate: "License plate",
      vehicle: "Vehicle",
      km: "Distance",
      averageSpeed: "Average speed",
      totalEventNumber: "Event counter",
      totalTimeTraveled: "Total time traveled",
      totalEngineHours: "Total engine hours",
    },
  },
  management: {
    tabs: {
      vehicles: "Vehicles",
      devices: "Devices",
      clients: "Clients",
      users: "Users",
      groups: "Groups",
      accounts: "Accounts",
      drivers: "Drivers",
    },
    tableColumns: {
      groupVehicles: "Group's vehicles",
      vehicleInformation: "Vehicle information",
      address: "Address",
      alias: "Alias",
      brand: "Brand",
      celphone: "Phone",
      createdAt: "Creation date",
      date: "Date",
      driver: "Driver",
      email: "Email",
      entryDate: "Entry date",
      firmware: "Firmware",
      groupName: "Group",
      hardware: "Hardware",
      id: "ID",
      imei: "IMEI",
      lastDataDate: "Last report date",
      lastnames: "Last name(s)",
      license: "License",
      model: "Model",
      name: "Name",
      names: "First name(s)",
      plates: "Plates",
      phoneNumber: "Phone number",
      registrationDate: "Registration date",
      serialNumber: "Serial number",
      sim: "SIM",
      status: "Status",
      type: "Device type",
      username: "Username",
      vehicleType: "Vehicle type",
      version: "Version",
      year: "Year",
    },
    dataProvider: {
      active: "Active",
      inactive: "Inactive",
    },
  },
  teleOBD: {
    resumes: {
      distance: "Total distance traveled",
      timeDriven: "Total driving time",
      timeIdle: "Total engine parked time",
    },
    tableColumns: {
      title: "Top 5",
      name: "Name",
      plate: "License plate",
      driverDistance: "Traveled Distance (Km)",
      averageSpeed: "Average speed (km/h)",
      driverTime: "Driving time (h)",
      driverIdleTime: "Engine parked time (h)",
      maxSpeed: "Max speed (km/h)",
    },
    tableTitle: {
      registerTeleOBD: "OBD Telemetry Record (On-Board Diagnostics)",
    },
    charts: {
      titleDistance: "Driving distance",
      titleDriven: "Driving time",
      titleIdle: "Engine parked time",
      subtitleDistance: "Median distance traveled",
      subtitleDriven: "Median driving time",
      average: "Average",
      subtitleIdle: "Median engine parked time",
      titleTooltipDistance: "Distance Traveled",
      titleTooltipDriven: "Driven Time",
      titleTooltipIdle: "Engine parked time",
      xAxisDistance: "Distance traveled (km)",
      xAxisDriven: "Driven time (h)",
      xAxisIdle: "Engine parked time (h)",
      yAxis: "Vehicles",
    },
  },
  zones: {
    zoneProfileDataShelf: {
      button: "Edit profile",
      title: "Profile data",
      zoneName: "Zone name",
      profileName: "Profile name",
    },
    zoneProfileForm: {
      name: "Name",
      category: "Category",
      charges: "Charges",
      discharges: "Discharges",
      ralenti: "Idle",
      color: "Color",
      provider: "Provider",
      description: "Description",
      authorized: "Authorized",
      noAuthorized: "Not authorized",
      selectAnOption: "Select an option",
    },
    zoneProfileCategories: {
      garage: "Garage", // TALLER
      gasStation: "Gas Station", // GASOLINERA
      restArea: "Rest Area", // DESCANSO
      headquarters: "Headquarters", // CORPORATIVO
      tollBooth: "Toll Booth", // CASETA
      selfConsumption: "Self-Consumption", // AUTOCONSUMO
      impoundLot: "Impound Lot", // CORRALÓN
      carWash: "Car Wash", // AUTOLAVADO
      privateProperty: "Private Property", // PARTICULAR
      parkingLot: "Parking Lot", // PENSIÓN
      branchOffice: "Branch Office", // SUCURSAL
      tireShop: "Tire Shop", // LLANTERA
      riskArea: "Risk Area", // LUGAR DE RIESGO
      fuelLoad: "Fuel Load", // CARGA DE COMBUSTIBLE
      mainOffice: "Main Office", // MATRIZ
      client: "Client", // CLIENTE
      supplier: "Supplier", // PROVEEDOR
      distributionCenter: "Distribution center", // CEDIS
      robbery: "Robbery", // ROBO
    },
    zonesFuelTable: {
      zoneModalTitle: "Zone map",
      zone: "Zone",
      profile: "Profile",
      country: "Country",
      state: "State / Province",
      city: "City",
      zipCode: "Postal code",
      events: "Total events",
      charged: "Liters loaded",
      discharged: "Liters unloaded",
    },
    zoneMap: {
      zonePopup: {
        address: "Address",
        magnitude: "Litres",
        initialFuel: "Initial L.",
        finalFuel: "Final L.",
        dateGps: "Date",
        startDate: "Start Date",
        endDate: "End Date",
        coordinates: "Coordinates",
      },
    },
    tabs: {
      map: "Map",
      load: "Load",
      unload: "Unload",
      loadTable: {
        vehicleId: "Vehicle",
        date: "Date",
        loadValue: "Load",
      },
      unloadTable: {
        vehicleId: "Vehicle",
        date: "Date",
        loadValue: "Unload",
      },
    },
  },
  iam: {
    tabs: {
      users: "Users",
      accounts: "Accounts",
      permissions: "Permissions",
      roles: "Roles",
    },
    users: {
      tableColumns: {
        email: "Email",
        userName: "User name",
        account: "Account",
        name: "Name",
        lastName: "Last name",
        createdAt: "Created at",
      },
      userForm: {
        saveChanges: "Save changes",
        deleteUser: "Delete user",
        deleteUserConfirmation: "Are you sure you want to delete this item?",
      },
    },
  },
};
